...
test.describe.serial('Attendance & Leave API', () => {
    ...
    test('Verify Apply Leave Employee - Super Admin', async ({ apiAsEmployee }) => {
    ...
    test('Verify Withdraw Leave', async ({ apiAsEmployee }) => {
        console.log('Withdrawing Leave ID:', laveId);

        const res = await apiAsEmployee.attendanceAndLeave.putWithdrawLeave(
            laveId,
            'check_withdrawal'
        );
